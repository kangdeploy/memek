...
▣ ${prefix}tiktokvideo _link_
▣ ${prefix}tiktokaudio _link_
▣ ${prefix}instagramdownload _link_
▣ ${prefix}instagramdl _link_
▣ ${prefix}igdownload _link_
▣ ${prefix}igdl link
▣ ${prefix}youtubemp4 _link_
...
case 'igstalk':
if (q.includes('--help')) return reply(examquery)
if (!q) return reply('*Example* : #igstalk Medy renaldy') 
     if (!isInventoryLimit){ addInventoriLimit(m.sender) }
     if (isLimit < 1) return reply("Limit kamu sudah habis !!, silahkan beli dengan cara #buy limit _jumlah_")
     kurangLimit(m.sender, 1)
     reply(`Satu limit terpakai !\nSisa limit kamu : ${getLimit(m.sender)}`)
...
for (let i of bc) {
sock.sendMessage(i.id, { text: `*[ FARBOT BROADCAST ]*\n\n${q}` })
}
break
case 'couple': {
if (q.includes('--help')) return reply(examkosong) 
...
if (q.includes('--help')) return reply(examkosong) 
  ...
  case 'menu2':{  
   ...
   let buttonss = [{
    ...
    from, 
    MenuList,
    `Whatsapp bot multi device coded by Fardev78`, 
    buttonss, 
    thumb, 
    await m.createMsg(
     from, 
...
m.reply(
    from, 
    '_*Perintah tersebut belum terdaftar dalam menu*_', 
    { 
     quoted : m 
    }
   )
